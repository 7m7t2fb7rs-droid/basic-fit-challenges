...
async function fetchAll() {
  ...
  inFlight = (async () => {
    const [ch, en, pa, se] = await Promise.all([
      ...
      supabase.from("settings").select("value").eq("key", COUNTDOWN_KEY).maybeSingle(),
    ]);
    ...
    const data = {
      ...
      // Le réglage est accessoire : s'il manque, la date par défaut suffit.
      countdownEnd: se.data?.value || DEFAULT_COUNTDOWN_END,
    };
    cache = data;
    cachedAt = Date.now();
    return data;
